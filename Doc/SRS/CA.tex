...
generator as well as the \famname{} family members it can generate. Ideally, 
the parent program knows the ODE(s) and other inputs at compile time, and
uses each generated family member many times to maximize the benefit of 
code generation. This is especially desirable if the ODE(s) or other inputs are 
no known at compile time; code generation can still happen at run-time, but 
ideally the parent program can offset the incurred cost of this on-the-fly 
generation.
...
a desired RK method, an interval and the desired number of knots, as well as an 
initial value, 

...
\\ 
${\bf f}$ & N/A & ODE for which to solve
\\
$h$ & N/A & step size
\\ 
${\bf k}_{1..4}$ & N/A & intermediate variables in the RK4 method, similar to 
${\bf Y}_1$
\\ 
...
\\ 
${\bf x(t)}$ & N/A & a function of $t$
\\ 
${\bf x}'$ & N/A & derivative of ${\bf x(t)}$
\\ 
${\bf x}_0$ & N/A & initial values for ${\bf f}$
\\ 
${\bf x}_k$ & N/A & (numerical approximation of) a point on the solution for 
${\bf f}$
\\ 
${\bf Y}_1$ & N/A & intermediate variables in the RK2 method, similar to ${\bf 
k}_{1..4}$
\\
\bottomrule
\end{longtable*}
...
\begin{minipage}{\textwidth}
    ...
    \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
        ...
        $[t_0,t_N]$ for which to find approximations; it essentially determines 
        how many points there will be once the interval is known. Once these 
        points have been approximated, one could interpolate so that the whole 
        interval is solved for.  %using RK methods (as described in 
...
instance models in \autoref{sec_Calculation}. Every unique input will 
result in slightly different code being generated, which could in turn produce 
different output compared to other family members. This allows for an 
infinite program family depending on one's definition of program 
family\footnote{Currently Dr. Smith and Dr. Carette agree they should discuss 
this definition to compare their differing viewpoints.}. After instance models, 
assumptions, calculation and output are covered.
...
\begin{minipage}{\textwidth}
    ...
    \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
        ...
        first step). Calculating a new point is necessary to create a spline to 

...
identifiers match most of the data definitions from the SRS. Note that $n$ is 
...
\begin{itemize}
  \item transition: N/A
  \item output: $ out := (\mathbb{N} \times \mathbb{R} \rightarrow \mathbb{R})$ 
  \texttt{code}
  \begin{itemize}
  ...
  \end{itemize}
  \item exception: $exc := (n < 2 \Rightarrow$ LESS\_THAN\_2\_KNOTS)
  \begin{itemize}
...
\begin{center}
	\begin{tabular}{p{5cm} p{3cm} p{3cm} p{2cm}}
		...
		\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
		\hline
		N/A - anonymous function & i : $\mathbb{N}, t : \mathbb{R}$ & $ x_i(t) : \mathbb{R}$ & - \\
		\hline
	\end{tabular}
\end{center}
...
\begin{itemize}
	...
	\end{itemize}
	\item exception: $exc := (i < t_0 | i > t_N \Rightarrow$ NOT\_IN\_RANGE)
\end{itemize}
